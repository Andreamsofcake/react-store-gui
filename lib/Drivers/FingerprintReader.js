...
class FingerprintReader {
	...
	flushGrabQ() {
		debug('flushGrabQ .... num queue items to flush: ' + grabQ.length);
		while (grabQ.length) {
			var obj = grabQ.pop();
			obj.reply({ token: obj.token, status: 'err', requestFlushed: true, apiResponse: 'request has been replaced by a new request' }).code(200);
		}
	}
	...
	*/
	...
	matchPrint(reply, matchProps, token) {
		...
		// ( if looking for admins, also look for super-sdk-admins :-)
		if (typeof matchUser === 'string' && matchUser === 'admin-') {
			matchUser = [ matchUser, 'sdkadmin-' ];
		}

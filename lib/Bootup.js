...
var filen = __filename.split(path.sep).pop()
	...
	, DELAY_TIME = parseInt(process.env.BOOTUP_DELAY_TIME_MS) || 60 * 5 * 1000
...
export function Register(cb, cascade, force) {
	...
	try {
		if (!force) {
			...
			if (json && json.success === true) {
				debug("\n\n" + 'registration exists.... continue [try block]' + "\n\n");
				if (!json.registrationData || !json.registrationData.client || !json.registrationData.location || !json.registrationData.teamViewer_id) {
					...
					debug("\n\n" + 'registration data incomplete, loop out to go get it in ' + (DELAY_TIME / 1000) + ' seconds' + "\n\n");
	...
	conn.query('select _value as serial from machinesettings where _key = "MachineSerialNumber"', (err, rows, fields) => {
		...
		if (rows && rows.length && rows[0] && rows[0].serial) {
			...
			debug("\n\n" + 'start post block to go get machine registration data .... ' + "\n\n");
			...
			RQ.post({
				...
				if (body && body.status == 'ok' && body.data) {
					...
					var D = new Date()
						, fileData = {
							success: true,
							machine_id: rows[0].serial,
							registrationData: body.data,
							datestamp: D.toString(),
							ts: Date.now()
						}
						;

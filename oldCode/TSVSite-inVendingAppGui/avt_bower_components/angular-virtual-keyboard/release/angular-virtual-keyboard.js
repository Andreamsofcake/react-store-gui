...
(function (angular) {
    ...
    var VKI = function(customConfig, layout, deadKeys, keyInputCallback) {
        ...
        self.attachVki = function(elem) {
            ...
            if (self.VKI_imageURI) {
                ...
                elem.onfocus = function() {
                    if (self.VKI_target != this) {
                        ...
                        self.VKI_show(this);
                        elem.setAttribute("VKI_isFocused", 'true');//Ping added on 0602/2015
                    }
                };


                //Ping added on 0603/2015
                elem.onblur = function() {
                    if (self.VKI_target != this) {
                        ...
                        self.VKI_close();
                        elem.setAttribute("VKI_isFocused", 'false');
                    }
                };
            ...
            VKI_addListener(elem, 'click', function(e) {
                if (self.VKI_target == this) {
                    ...
                    angular.forEach( angular.element(document).find('input'), function(value) {
                        ...
                        if(angular.equals('true', inputChild.attr('VKI_attached')) && !angular.equals(elem, value)) {
                            if(angular.equals('true', inputChild.attr('VKI_isFocused'))){
                                inputChild.triggerHandler('close');
                                inputChild.triggerHandler('submit');
                                value.setAttribute("VKI_isFocused", 'false');
                                inputChild.triggerHandler('blur2');
                            }
                        }
                    });
                }
            ...
            //VKI_addListener(document.documentElement, 'click', function(e) { self.VKI_close(); console.log("close!");}, false);

            angular.element(elem).bind('submit', function(){ self.VKI_enterSubmit();console.log("submit"); });
            angular.element(elem).bind('blur2', function() {
                this.blur();
            });
        };
    ...
    angular.module('angular-virtual-keyboard', [])
        ...
        .service('avtVirtualKeyboardService', ['AVT_VKI_CONFIG', function(AVT_VKI_CONFIG) {
            /*globals VKI */
            return {
                attach: function(element, config, inputCallback) {
                    config = config || {};
                    config.i18n = config.i18n || AVT_VKI_CONFIG.i18n;
                    config.kt = config.kt || AVT_VKI_CONFIG.kt;
                    config.relative = config.relative === false ? false : AVT_VKI_CONFIG.relative;
                    config.keyCenter = config.keyCenter || AVT_VKI_CONFIG.keyCenter;

                    var vki = new VKI(config, AVT_VKI_CONFIG.layout, AVT_VKI_CONFIG.deadkey, inputCallback);
                    vki.attachVki(element);
                }
            };
        }])
        .directive('avtVirtualKeyboard', ['avtVirtualKeyboardService', '$timeout', '$injector',
            function(avtVirtualKeyboardService, $timeout, $injector) {
                return {
                    restrict: 'A',
                    require : '?ngModel',
                    scope: {
                        config: '=avtVirtualKeyboard'
                    },
                    link: function(scope, elements, attrs, ngModelCtrl) {
                        ...
                        avtVirtualKeyboardService.attach(elements[0], scope.config, function() {
                            $timeout(function() {
